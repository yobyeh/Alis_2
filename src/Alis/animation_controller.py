...
class AnimationController(threading.Thread):
    def __init__(self, stop_evt, frame_queue,
                current_settings, settings_lock, web_animation_queue, show_animation_qeue, show_entry_complete_event, interface_animation_queue=None):
        ...
        # interface to animation controller queue (future use)
        ...
        try:
            while not self.shutdown_event.is_set():
                ...
                match self.mode:
                    ...
                    case "animation":
                        print("animation controller: animation")
                        if msg and isinstance(msg, dict):
                            ...
                            if self.text_name:
                                ...
                                for entry in text_entries:
                                    if entry.get("name") == self.text_name:
                                        ...
                                        break
                            if text_params:
                                self.text_height = int(text_params.get("height", 50))
                                self.text_width = int(text_params.get("width", 14))
